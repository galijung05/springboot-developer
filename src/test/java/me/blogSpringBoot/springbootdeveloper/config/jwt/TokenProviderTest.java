package me.blogSpringBoot.springbootdeveloper.config.jwt;
...
public class TokenProviderTest {
    ...
    void generateToken() {
        ...
        User testUser = userRepository.save(User.builder()
                ...
                .password("test")
                .build());
        // when (토큰 제공자의 generateToken()를 호출해 토큰 생성)
        String token = tokenProvider.generateToken(testUser, Duration.ofDays(14));

        // then (jjwt 라이브러리를 활용해 토큰 복호화, 토큰을 만들 때 클레임으로 넣어둔 id값이 given절에서 만든 유저 ID와 동일한지 확인)
    ...
    void validToken_invalidToken() {
        ...
        // when (토큰 제공자의 validToken() 메서드를 호출해 유효한 토큰인지 검증)
    ...
    void validToken_validToken() {
        // given (jjwt 라이브러리를 사용해 토큰 생성, 만료되지 않은 토큰으로 생성)
    ...
    void getAuthentication() {
        // given (jjwt 라이브러리를 사용해 토큰 생성, userEmail을 토큰의 제목으로 사용)
    ...
    @Test
    void getUserId() {
        // given (jjwt 라이브러리를 사용해 토큰 생성, 키는 "id, 값은 1이라는 클레임 추가)
         Long userId = 1L;
        String token = JwtFactory.builder()
